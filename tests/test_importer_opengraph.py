...
def test_filter_companion_properties_drops_unmatchable_well_known_principals():
    importer = BloodhoundImporter(None)
    data = _opengraph(
        "Groups",
        [
...
def test_dedupe_mssql_servers_iterates_each_relationship_type():
    conn = RecordingConnection(
        responses=[
            [{"canonical": "real:1433", "stale": ["stub:1433"]}],
            [],
            [],
            [{"rtype": "MSSQL_Contains"}, {"rtype": "MSSQL_LinkedTo"}],
            [], [],
            [], [],
            [],
        ]
    ...
    rel_types = {"MSSQL_Contains", "MSSQL_LinkedTo"}
    for rtype in rel_types:
        per_type = [q for q, _ in conn.queries if f"-[new:`{rtype}`]" in q]
        assert len(per_type) == 2, f"expected outbound+inbound merge for {rtype}"
...
def test_is_mssql_loopback_id_matches_known_loopback_forms(object_id):
    assert BloodhoundImporter._is_mssql_loopback_id(object_id) is True
...
def test_is_mssql_loopback_id_rejects_non_loopback_forms(object_id):
    assert BloodhoundImporter._is_mssql_loopback_id(object_id) is False


def _make_canonical_payload(nodes, edges):
    return {'graph': {'nodes': nodes, 'edges': edges}}


def test_canonicalise_mssql_payload_rewrites_instance_form_source():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'S-1-5-21-4806:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
            {'id': 'S-1-5-21-1105:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
        ],
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-4806:SQL01'},
             'end': {'value': 'S-1-5-21-1105:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (0, 1)
    edge = payload['graph']['edges'][0]
    assert edge['start']['value'] == 'S-1-5-21-4806:1433'
    assert edge['end']['value'] == 'S-1-5-21-1105:1433'


def test_canonicalise_mssql_payload_skips_ambiguous_multi_instance_sid():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'S-1-5-21-HOST:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
            {'id': 'S-1-5-21-HOST:1434', 'kinds': ['MSSQL_Server'], 'properties': {}},
            {'id': 'S-1-5-21-TGT:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
        ],
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-HOST:INST2'},
             'end': {'value': 'S-1-5-21-TGT:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (0, 0)
    assert payload['graph']['edges'][0]['start']['value'] == 'S-1-5-21-HOST:INST2'


def test_canonicalise_mssql_payload_leaves_declared_endpoints():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'S-1-5-21-4806:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
        ...
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-4806:1433'},
             'end': {'value': 'S-1-5-21-4806:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (0, 0)
...
def test_canonicalise_mssql_payload_returns_zero_without_mssql_servers():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[{'id': 'foo', 'kinds': ['Other'], 'properties': {}}],
        edges=[{'kind': 'X', 'start': {'value': 'a:b'}, 'end': {'value': 'c:d'}, 'properties': {}}],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (0, 0)


def test_canonicalise_mssql_payload_resolves_loopback_via_has_links_from_servers():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'S-1-5-21-4806:1433', 'kinds': ['MSSQL_Server'], 'properties': {'name': 'lab-sql01:1433'}},
            {'id': 'localhost:1433', 'kinds': ['MSSQL_Server'],
             'properties': {'name': 'localhost', 'hasLinksFromServers': ['S-1-5-21-4806:1433']}},
        ],
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-4806:SQL01'},
             'end': {'value': 'localhost:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (1, 2)
    node_ids = [n['id'] for n in payload['graph']['nodes']]
    assert node_ids == ['S-1-5-21-4806:1433']
    edge = payload['graph']['edges'][0]
    assert edge['start']['value'] == 'S-1-5-21-4806:1433'
    assert edge['end']['value'] == 'S-1-5-21-4806:1433'


def test_canonicalise_mssql_payload_leaves_ambiguous_loopback_alone():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'localhost:1433', 'kinds': ['MSSQL_Server'],
             'properties': {'hasLinksFromServers': ['S-1-5-21-A:1433', 'S-1-5-21-B:1433']}},
        ],
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-A:1433'},
             'end': {'value': 'localhost:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (0, 0)
    assert payload['graph']['nodes'][0]['id'] == 'localhost:1433'
    assert payload['graph']['edges'][0]['end']['value'] == 'localhost:1433'


def test_canonicalise_mssql_payload_handles_multiple_stubs_to_same_canonical():
    imp = BloodhoundImporter.__new__(BloodhoundImporter)
    payload = _make_canonical_payload(
        nodes=[
            {'id': 'S-1-5-21-4806:1433', 'kinds': ['MSSQL_Server'], 'properties': {}},
            {'id': 'localhost:1433', 'kinds': ['MSSQL_Server'],
             'properties': {'hasLinksFromServers': ['S-1-5-21-4806:1433']}},
        ],
        edges=[
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-4806:INST1'},
             'end': {'value': 'localhost:1433'},
             'properties': {}},
            {'kind': 'MSSQL_LinkedTo',
             'start': {'value': 'S-1-5-21-4806:INST2'},
             'end': {'value': 'localhost:1433'},
             'properties': {}},
        ],
    )
    assert imp._canonicalise_mssql_payload(payload, 'test') == (1, 4)
    assert [n['id'] for n in payload['graph']['nodes']] == ['S-1-5-21-4806:1433']
    for edge in payload['graph']['edges']:
        assert edge['start']['value'] == 'S-1-5-21-4806:1433'
        assert edge['end']['value'] == 'S-1-5-21-4806:1433'


def test_canonicalise_companion_sid_endpoints_uppercases_authusers():
    payload = _make_canonical_payload(
        nodes=[],
        edges=[
            {'kind': 'CoerceAndRelayToMSSQL',
             'start': {'value': 'training.local-S-1-5-11'},
             'end': {'value': 'training\\@S-1-5-21-1105:1433'},
             'properties': {}},
    ...
    assert BloodhoundImporter._canonicalise_companion_sid_endpoints(payload) == 2
    starts = [edge['start']['value'] for edge in payload['graph']['edges']]
    assert starts == ['TRAINING.LOCAL-S-1-5-11', 'TRAINING.LOCAL-S-1-5-11']


def test_canonicalise_companion_sid_endpoints_leaves_canonical_alone():
    payload = _make_canonical_payload(
        nodes=[],
        edges=[
            {'kind': 'CoerceAndRelayToMSSQL',
            ...
            {'kind': 'SCCM_IsMappedTo',
             'start': {'value': 'S-1-5-18'},
             'end': {'value': 'NT AUTHORITY\\SYSTEM@TRN'},
             'properties': {}},
        ],
    )
    assert BloodhoundImporter._canonicalise_companion_sid_endpoints(payload) == 0
    assert payload['graph']['edges'][0]['start']['value'] == 'TRAINING.LOCAL-S-1-5-11'
    assert payload['graph']['edges'][1]['start']['value'] == 'S-1-5-18'
