...
class HTMLReportGenerator(PasswordAuditMixin, PolicyComplianceMixin, CrossDomainMixin, UserAccountAnalysisMixin):
        ...
        for user_id in displayed_users:
            if user_id in user_details:
                ...
                tooltip_data[user_id] = {
                    ...
                    'isPrivileged': user_data.get('isPrivileged', False),
    ...
    def generate_html_content_with_tabs(self, report_data, password_length_data, filename, css_content, js_content):
        ...
        html_lines.append(f'    <title>Password Analysis Report for {self.domain_name}</title>')
        ...
        html_lines.append(f'    <h1>Password Analysis Report for {self.domain_name}</h1>')
        ...
        html_lines.append(f'    <script>var outputFormat = "{self.output_format}";</script>')
        html_lines.append(f'    <script>var stats = {json.dumps(sanitized_stats)};</script>')
        html_lines.append(f'    <script>var passwordLengthData = {json.dumps(sanitized_password_length_data)};</script>')
        html_lines.append(f'    <script>var userDetails = {json.dumps(sanitized_user_details)};</script>')
        html_lines.append(f'    <script>var highValueGroupsList = {json.dumps(sorted(HIGH_VALUE_GROUPS_UPPER))};</script>')
        html_lines.append(f'    <script>var domainName = "{self.domain_name}";</script>')
        html_lines.append(f'    <script>var domainPolicy = {json.dumps(sanitized_domain_policy)};</script>')
        html_lines.append(f'    <script>var complianceStats = {json.dumps(sanitized_compliance_stats)};</script>')
        html_lines.append(f'    <script>var violationDistribution = {json.dumps(sanitized_violation_distribution)};</script>')
        
        sanitized_violations_data = sanitize_data_for_safe_mode(self._build_policy_violations_data(), self.output_format)
        html_lines.append(f'    <script>var policyViolationsData = {json.dumps(sanitized_violations_data)};</script>')
        html_lines.append('    <script>')
        ...
        if not user_data:
            ...
            if sanitised_username == 'krbtgt' and self.neo4j_data:
                ...
                if krbtgt_pwd and krbtgt_pwd not in ['Unknown', 'Never', 0, -1]:
                    pwd_last_changed = krbtgt_pwd  # Store raw timestamp, format_timestamp will format it
    ...
    def generate_tooltip_content(self, user_data):
        formatted_groups = []
        for group in user_data.get('groups', []):
            if any(hvg in group.upper() for hvg in HIGH_VALUE_GROUPS_UPPER):
                formatted_groups.append(f"<strong>{html.escape(group)}</strong>")
            else:
                formatted_groups.append(html.escape(group))
        
        content = f"""
            <strong>Domain:</strong> {user_data.get('domain', 'N/A')}<br>
            <strong>Username:</strong> {user_data.get('username', 'N/A')}<br>
            <strong>Description:</strong> {user_data.get('description', 'N/A')}<br>
            <strong>Enabled:</strong> {'Yes' if user_data.get('enabled') else 'No'}<br>
            <strong>Admin:</strong> {'Yes' if user_data.get('isAdmin') else 'No'}<br>
            <strong>Privileged:</strong> {'Yes' if user_data.get('isPrivileged') else 'No'}<br>
        """
        
        if self.output_format == 'unsafe' and (user_data.get('isAdmin') or user_data.get('isPrivileged')):
            content += f"<strong>Privileged Access:</strong> {user_data.get('adminAccessReason', 'N/A')}<br>"
        
        content += f"""
            <strong>Last Logon:</strong> {self.format_timestamp(user_data.get('lastLogon', 'N/A'))}<br>
            <strong>Password Last Changed:</strong> {self.format_timestamp(user_data.get('passwordLastChanged', 'N/A'))}<br>
            <strong>Groups:</strong> {', '.join(formatted_groups)}
        """

        if self.output_format == 'unsafe':
            shared_with = user_data.get('sharedWith') or []
            if shared_with:
                shared_list = ', '.join(html.escape(u) for u in shared_with)
                content += f"<br><strong>Shared with:</strong> {shared_list}"
        ...
        if self.output_format == 'unsafe':
            ...
            if password_value == '':
                password_display_text = "(blank)"
            elif password_value is None:
                password_display_text = "Not cracked"
            else:
                password_display_text = html.escape(password_value)

            content += f"<br><strong>Password:</strong> {password_display_text}"

        return content.replace('\n', '').strip().replace('"', '"')
    
    def format_timestamp(self, timestamp):
        if timestamp in ['Never', 'Unknown']:
            return timestamp
        try:
    ...
    def generate_password_statistics_content(self, password_length_data):
        ...
        if min_pwd_length is not None:
            ...
            content.append(f'                        <span>Meets requirement (≥ {min_pwd_length} characters)</span>')
            content.append('                    </div>')
            content.append('                </div>')
            content.append('            </div>')
        ...
        content.append(f'            <script>var chartData = {json.dumps(chart_data)};</script>')
