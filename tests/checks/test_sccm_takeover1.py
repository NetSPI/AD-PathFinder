...
def test_fires_on_mssql_relay_to_sccm_db(clean_neo4j):
    ...
    assert SCCMTakeover1Check.RISK_LEVEL == "Critical"
    assert len(findings) == 1
    assert "S-1-5-21-TEST-1700" in findings
    assert isinstance(findings["S-1-5-21-TEST-1700"], str)


def test_excludes_foreign_and_disabled_coercion_sources(clean_neo4j):
    load_fixture(clean_neo4j, "sccm_takeover1_source_filter.cypher")
