...
Keys off the Distribution Point role in SCCMSiteSystemRoles rather than
SCCMIsPXESupportEnabled alone — the PXE flag only reflects REMINST share
presence (SMB collection) and misses DHCP-based PXE (options 66/67).
...
class SCCMPxeDPCheck(Check):
    ...
    def execute(self):
        ...
        for dp in dps:
            ...
            pxe_confirmed = dp.get('pxe_confirmed', False)
            desc = hostname
            if site_code:
    ...
    def _fetch_distribution_points(self):
        ...
        for row in rows:
            ...
            dps.append({
                ...
                'hostname': hostname,
                'site_code': site_code,
                'pxe_confirmed': bool(row.get('pxe_confirmed')),
            })
        return dps
