...
def _zip_is_supplementary_only(zip_path):
    import zipfile
    import json
        ...
        with zipfile.ZipFile(Path(zip_path), 'r') as zf:
            ...
            for json_file in json_files:
                ...
                if (
                    ...
                    or not isinstance(data.get('graph'), dict)
                ):
                    return False
        return True
    except Exception:
...
def _is_supplementary_only_import(zip_paths):
    import_files = zip_paths if isinstance(zip_paths, list) else [zip_paths]
    return bool(import_files) and all(_zip_is_supplementary_only(f) for f in import_files)
...
def handle_import_with_check(args, importer, conn):
    ...
    is_supplementary_only = _is_supplementary_only_import(import_files)
    has_existing_data = check_database_has_data(conn)

    if is_supplementary_only and not has_existing_data:
        print("[ERROR] OpenGraph plugin data requires existing BloodHound/SharpHound data. Import SharpHound first or include it in the same import command.")
        return False

    if has_existing_data:
        if is_supplementary_only:
            print("[INFO] Importing supplementary OpenGraph data")
            print("[INFO] Supplementary data will be added to existing BloodHound data")
        else:
    ...
    data_type = "supplementary data" if is_supplementary_only else "BloodHound data"
...
def main():
    ...
    parser.add_argument('-i', '--import', dest='import_file', type=str, nargs='+', help='Import BloodHound CE data from one or more zip files')
    parser.add_argument('-d', '--delete', action='store_true', help='Delete all BloodHound CE data and reset the database')
    ...
    parser.add_argument('-m', '--mark_owned', action='store_true', help='Mark accounts with cracked passwords as owned in BloodHound CE and store passwords in Neo4j, displayed in the BloodHound GUI')
