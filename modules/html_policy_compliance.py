    ...
    def generate_policy_compliance_content(self):
        ...
        if stats["length_violations"] > 0 and min_length is not None:
            tooltip_length = self._create_metrics_tooltip(
                ...
                f"Passwords below the minimum length threshold of {min_length} characters.",
        ...
        if min_pwd_length_policy is not None and min_pwd_length_policy > 0 and stats.get('length_violations', 0) > 0:
            content.append(f'<li><strong>+1</strong> Password below length threshold ({min_pwd_length_policy} characters)</li>')
            violations_shown += 1

        if len(admin_users) > 0:
            content.append('<li><strong>+1</strong> Admin account</li>')
            violations_shown += 1
