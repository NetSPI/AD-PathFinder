...
def get_valid_bloodhound_credentials(
    ...
    *,
    ...
    while attempts < max_attempts:
        username = input_fn("If using BloodHoundCE, provide BloodHound username (or press Enter to skip): ").strip()
        if not username:
            print("Skipping BloodHound integration as per user choice.")
            disable_bloodhound(config, config_file, base_url)
            return None, None

        password = input_fn("If using BloodHoundCE, provide BloodHound password (or press Enter to skip): ").strip()
        if not password:
            print("Skipping BloodHound integration as per user choice.")
            disable_bloodhound(config, config_file, base_url)
            return None, None
