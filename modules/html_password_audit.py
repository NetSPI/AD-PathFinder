...
class PasswordAuditMixin:
    def generate_password_audit_content(self, report_data):
        ...
        indirect_access_tooltip = (
            "<b>User has permissions that allow modification of privileged groups or their members:</b><br><br>"
            "• WriteDacl/WriteOwner/GenericWrite/GenericAll permissions on privileged groups<br>"
            "• WriteDacl/WriteOwner/GenericWrite/GenericAll permissions on privileged group members<br>"
            "• Control of Group Policy Objects linked to privileged OUs<br>"
            "• Ability to change service configurations on domain controllers"
        )
        ...
        for line_num, line in enumerate(report_data.split('\n')):
            ...
            elif line.startswith('Cracked Privileged Accounts (Indirect Access):'):
                new_section_detected = True
                new_current_section_name = 'cracked_privileged'
                count = line.split(':')[-1].strip()
                tooltip_html = self._create_hoverable_tooltip_span("(Indirect Access)", indirect_access_tooltip, style="color: inherit; text-decoration: none; cursor: help")
                section_header_html = f'        <h2>Cracked Privileged Accounts {tooltip_html}: {count}</h2>'
