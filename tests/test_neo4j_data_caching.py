...
class TestGetAllUsersSignatureMatchesLiveCallers(unittest.TestCase):
    """Function must still accept every kwarg today's callers pass:
      - account_analysis.py -> force_refresh=...
      - user_query.py       -> username=...
    ...
    def test_signature_accepts_known_caller_kwargs(self):
        ...
        for kw in ("username", "force_refresh"):
    ...
    def test_force_refresh_kwarg_shape_works(self):
        nd = Neo4jData(MagicMock(), domain_filter="A.LOCAL")
        nd.all_users_data_cache = [{"username": "alice"}]
        result = nd.get_all_users_with_attributes(force_refresh=False)
        self.assertEqual(result, [{"username": "alice"}])
