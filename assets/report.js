...
function setupPasswordLengthChart() {
    ...
    const backgroundColors = labels.map(length => {
        ...
        return parseInt(length) < minPasswordLength ?
            "rgba(220, 53, 69, 0.6)" : // Red for below the minimum
            "rgba(54, 162, 235, 0.6)"; // Blue for meets requirement
    });
    ...
    const borderColors = labels.map(length => {
        ...
        return parseInt(length) < minPasswordLength ?
            "rgba(220, 53, 69, 1)" : // Red for below the minimum
            "rgba(54, 162, 235, 1)"; // Blue for meets requirement
    });
...
function generateTooltip(username) {
    ...
    if (outputFormat === 'unsafe' && userDetail.isAdmin) {
        content += `<strong>Admin Access:</strong> ${escapeHtml(userDetail.adminAccessReason || 'N/A')}<br>`;
    }
