...
from modules.node_type_cache import ad_reportable_labels_from_labels


@CrossDomainRegistry.register
class CrossDomainComputerEscalationPathCheck(CrossDomainCheck):
    RISK_LEVEL = "High"
    CATEGORY_NAME = "Cross-Domain Computer Escalation Paths"
    REQUIRES_NTDS = False
    BATCH_SIZE = 250
    ...
    def execute(self):
        ...
        computer_sids = self._get_source_sids()
        if not computer_sids:
            return []

        num_workers = self.deps.get_adaptive_workers(len(computer_sids))

        da_findings = self._query_paths_batched(
            computer_sids, rels, 'da', num_workers
        )
        computers_with_da = {f['source_sid'] for f in da_findings}

        remaining_sids = [sid for sid in computer_sids if sid not in computers_with_da]
        tier0_findings = []
        if remaining_sids:
    ...
    def _get_source_sids(self):
        query = """
            MATCH (c:Computer)
            WHERE NOT "Tag_Tier_Zero" IN labels(c)
              AND NOT (c.system_tags IS NOT NULL AND c.system_tags CONTAINS 'admin_tier_0')
              AND coalesce(c.highvalue, false) = false
            RETURN c.objectid AS sid
        """
        try:
            results = self.deps.conn.query(query, name="cross_domain_computer_paths_source_sids")
            return [r['sid'] for r in results if r.get('sid')]
        except Exception as e:
            if self.deps.diagnostics:
                self.deps.diagnostics.record_error("cross_domain:computer_paths.source_sids", e)
            return []
                    ...
                    try:
                        ...
                        if self.deps.diagnostics:
                            self.deps.diagnostics.record_error(
                                "cross_domain:computer_paths.worker_future", e)
    ...
    def _query_batch(self, batch_sids, rels, target_type, conn=None):
        ...
        if target_type == 'da':
            query = f'''
                ...
                RETURN source.name AS source_computer,
            ...
            query = f'''
                ...
                RETURN source.name AS source_computer,
                       ...
                       shortest.len AS path_length
            '''
            result_target_type = 'High-value target'

        try:
            results = conn.query(query, {'batch_sids': batch_sids},
                                 name=f"cross_domain_computer_paths_{target_type}")
            return self._process_results(results, target_type=result_target_type)
        except Exception as e:
            if self.deps.diagnostics:
                self.deps.diagnostics.record_error(
                    f"cross_domain:computer_paths.batch_{target_type}", e)
            return []
    ...
    def _process_results(self, results, target_type='Unknown'):
        ...
        for row in results:
            path_nodes = [
                {**node, 'labels': ad_reportable_labels_from_labels(node.get('labels') or [])}
            ...
            findings.append({
                'source_computer': row.get('source_computer') or '',
    ...
    def to_display_results(cls, findings, source_domain=None):
        ...
        path_to_computers = defaultdict(set)
        ...
        for f in findings:
            ...
            if normalized:
                computer_name = f.get('source_computer') or ''
                if '.' in computer_name:
                    computer_display = computer_name.split('.')[0]
                else:
                    computer_display = computer_name
                path_to_computers[normalized].add(computer_display)

        if not path_to_computers:
            return {}

        sorted_paths = sorted(path_to_computers.items(), key=lambda x: len(x[1]), reverse=True)

        results = {}
        for normalized_path, computers in sorted_paths:
            sorted_computers = sorted(computers, key=str.lower)
            computers_str = ", ".join(sorted_computers)
            key = f"Computers with Shared Path ({len(computers)}): {computers_str}"
            results[key] = f"Cross-Domain Path: {normalized_path}"

        total_computers = len(set(c for computers in path_to_computers.values() for c in computers))
        results['__total_user_count__'] = total_computers
