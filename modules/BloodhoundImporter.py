...
_UNMATCHABLE_COMPANION_PRINCIPAL_IDS = {
    "S-1-5-7",
    "S-1-5-11",
    "S-1-5-18",
    "S-1-5-19",
...
def _is_unmatchable_companion_principal_id(value: object) -> bool:
    object_id = str(value or "").strip().upper()
    if not object_id:
        return False
    return (
        object_id in _UNMATCHABLE_COMPANION_PRINCIPAL_IDS
        or object_id.startswith("S-1-5-32-")
        or object_id.endswith("-S-1-5-11")
    )
...
class BloodhoundImporter:
    ...
    def _filter_companion_properties(
        ...
        for node in nodes:
            if not set(node.get('kinds', [])).issubset(permitted_kinds):
                ...
                continue
            if _is_unmatchable_companion_principal_id(node.get('id')):
                continue
    ...
    def _import_opengraph_files_directly(
        ...
        *,
        ...
        for name, body, merge_mode in payloads:
            ...
            if merge_mode == 'opengraph':
                resolved, rewritten = self._canonicalise_mssql_payload(data, name)
                if resolved or rewritten:
                    print(
                        f"[*] OpenGraph {name}: canonicalised "
                        f"{resolved} loopback MSSQL_Server node(s) and "
                        f"{rewritten} edge endpoint(s)"
                    )
                sid_rewritten = self._canonicalise_companion_sid_endpoints(data)
                if sid_rewritten:
                    print(
                        f"[*] OpenGraph {name}: canonicalised "
                        f"{sid_rewritten} Authenticated Users edge endpoint(s)"
                    )
                graph = data.get('graph', {})
    ...
    @staticmethod
    def _is_mssql_loopback_id(object_id: str) -> bool:
        return object_id.lower().startswith(_MSSQL_LOOPBACK_ID_PREFIXES)

    @staticmethod
    def _canonicalise_companion_sid_endpoints(data: dict) -> int:
        # ConfigManBearPig lowercases the domain on <domain>-S-1-5-11; SharpHound
        # uppercases it. Without this, the relay edges hang off an orphan stub.
        ...
        for edge in edges:
            for side in ('start', 'end'):
                ...
                if canonical != value and canonical.endswith('-S-1-5-11'):
                    endpoint['value'] = canonical
                    rewritten += 1
        return rewritten

    def _canonicalise_mssql_payload(self, data: dict, source_name: str) -> Tuple[int, int]:
        # MSSQLHound emits loopback stubs (localhost:1433) tagged with
        # hasLinksFromServers=[canonical], and edges sometimes use SID:INST while
        # the declared node is SID:1433. Reconcile both before insert.
        graph = data.get('graph') or {}
        nodes = graph.get('nodes') or []
        edges = graph.get('edges') or []
        declared_ids: Set[str] = set()
        sid_candidates: Dict[str, Set[str]] = {}
        for node in nodes:
            oid = node.get('id')
            if not isinstance(oid, str):
                continue
            declared_ids.add(oid)
            if 'MSSQL_Server' not in (node.get('kinds') or []):
                continue
            if ':' not in oid or self._is_mssql_loopback_id(oid):
                continue
            sid_candidates.setdefault(oid.split(':', 1)[0], set()).add(oid)
        canonical_by_sid = {
            sid: next(iter(ids)) for sid, ids in sid_candidates.items() if len(ids) == 1
        }

        loopback_remap: Dict[str, str] = {}
        rewritten_nodes: List[dict] = []
        for node in nodes:
            object_id = node.get('id')
            if (
                not isinstance(object_id, str)
                or 'MSSQL_Server' not in (node.get('kinds') or [])
                or not self._is_mssql_loopback_id(object_id)
            ):
                rewritten_nodes.append(node)
                continue
            properties = node.get('properties') or {}
            canonicals = properties.get('hasLinksFromServers')
            if not isinstance(canonicals, list) or len(canonicals) != 1:
                rewritten_nodes.append(node)
                continue
            canonical = canonicals[0]
            if not isinstance(canonical, str) or canonical == object_id:
                rewritten_nodes.append(node)
                continue
            loopback_remap[object_id] = canonical
            if canonical in declared_ids:
                continue
            declared_ids.add(canonical)
            carry = {k: v for k, v in properties.items() if k != 'name'}
        ...
        if loopback_remap or canonical_by_sid:
            for edge in edges:
                for side in ('start', 'end'):
                    endpoint = edge.get(side)
                    if not isinstance(endpoint, dict):
                    ...
                    if not isinstance(value, str):
                        continue
                    if value in loopback_remap:
                        endpoint['value'] = loopback_remap[value]
                        endpoints_rewritten += 1
                        continue
                    if ':' not in value or value in declared_ids:
                        continue
                    canonical = canonical_by_sid.get(value.split(':', 1)[0])
                    if canonical and canonical != value:
                        endpoint['value'] = canonical
                        endpoints_rewritten += 1
        return len(loopback_remap), endpoints_rewritten
