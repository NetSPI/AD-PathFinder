...
def test_fires_on_distribution_point(clean_neo4j):
    ...
    assert SCCMPxeDPCheck.RISK_LEVEL == "Medium"
    assert len(findings) == 1
    assert "S-1-5-21-TEST-1400" in findings
    assert "S-1-5-21-TEST-1401" not in findings
    assert isinstance(findings["S-1-5-21-TEST-1400"], str)
    assert "PXE confirmed via REMINST" in findings["S-1-5-21-TEST-1400"]
